...
ALL_APPLICATION_WINDOW_IDS_EXPRESSION="map({app, id }) | group_by(.app) | map({app: .[0].app, ids: map(.id)}) | map(select(.app == $CURRENT_WIN_NAME))[0] | .ids"
ALL_APPLICATION_WINDOW_IDS=$(yabai -m query --windows | jq "$ALL_APPLICATION_WINDOW_IDS_EXPRESSION")
GET_NEXT_ID=".[index($CURRENT_WIN_ID) + 1]"
